    ...
    if (wstate->other_modes.sample_type || wstate->other_modes.en_tlut) {
        ...
        if (bilerp) {
            ...
            if (!wstate->other_modes.sample_type)
                ...
                fetch_texel_entlut_quadro(wstate, &t0, &t1, &t2, &t3, sss1, sdiff, sst1, tdiff, tilenum, upper,
                                          upperrg);
            else
            ...
            if (!convert) {
                ...
                if (!centerrg) {
                    ...
                    TEX->r =
                        prevb + ((prevr * (t2.r - t3.r) + prevg * (t1.r - t3.r) + ((invt3r + t0.r) << 6) + 0xc0) >> 8);
                    TEX->g =
                        prevb + ((prevr * (t2.g - t3.g) + prevg * (t1.g - t3.g) + ((invt3g + t0.g) << 6) + 0xc0) >> 8);
                }
                ...
                if (!center) {
                    ...
                    TEX->b =
                        prevb + ((prevr * (t2.b - t3.b) + prevg * (t1.b - t3.b) + ((invt3b + t0.b) << 6) + 0xc0) >> 8);
                    TEX->a =
                        prevb + ((prevr * (t2.a - t3.a) + prevg * (t1.a - t3.a) + ((invt3a + t0.a) << 6) + 0xc0) >> 8);
                }
            }
        } else {
...
static void
loading_pipeline(struct rdp_state *wstate, int start, int end, int tilenum, int coord_quad, int ltlut)
{
            ...
            switch (tiptr & 7) {
                case 0:
                    if (!ltlut)
                        ...
                        loadqword = ((uint64_t)tempshort << 48) | ((uint64_t)tempshort << 32) |
                                    ((uint64_t)tempshort << 16) | tempshort;
                    }
                    break;
                case 1:
