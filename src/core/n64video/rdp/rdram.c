...
static STRICTINLINE bool
rdram_valid_idx8(uint32_t in)
{
    return in <= idxlim8;
}

static STRICTINLINE bool
rdram_valid_idx16(uint32_t in)
{
    return in <= idxlim16;
}

static STRICTINLINE bool
rdram_valid_idx32(uint32_t in)
{
    return in <= idxlim32;
}

static STRICTINLINE uint8_t
rdram_read_idx8(uint32_t in)
{
    in &= RDRAM_MASK;
...
static STRICTINLINE uint8_t
rdram_read_idx8_fast(uint32_t in)
{
    return rdram8[in ^ BYTE_ADDR_XOR];
}

static STRICTINLINE uint16_t
rdram_read_idx16(uint32_t in)
{
    in &= RDRAM_MASK >> 1;
...
static STRICTINLINE uint16_t
rdram_read_idx16_fast(uint32_t in)
{
    return rdram16[in ^ WORD_ADDR_XOR];
}

static STRICTINLINE uint32_t
rdram_read_idx32(uint32_t in)
{
    in &= RDRAM_MASK >> 2;
...
static STRICTINLINE uint32_t
rdram_read_idx32_fast(uint32_t in)
{
    return rdram32[in];
}

static STRICTINLINE void
rdram_write_idx8(uint32_t in, uint8_t val)
{
    in &= RDRAM_MASK;
...
static STRICTINLINE void
rdram_write_idx16(uint32_t in, uint16_t val)
{
    in &= RDRAM_MASK >> 1;
...
static STRICTINLINE void
rdram_write_idx32(uint32_t in, uint32_t val)
{
    in &= RDRAM_MASK >> 2;
...
static STRICTINLINE void
rdram_read_pair16(uint16_t *rdst, uint8_t *hdst, uint32_t in)
{
    in &= RDRAM_MASK >> 1;
    if (rdram_valid_idx16(in)) {
...
static STRICTINLINE void
rdram_write_pair8(uint32_t in, uint8_t rval, int flip, int *delayedhbwidx)
{
    in &= RDRAM_MASK;
    if (!flip) {
        if (rdram_valid_idx8(in)) {
...
static STRICTINLINE void
rdram_write_pair16(uint32_t in, uint16_t rval, uint8_t hval, int iscolor)
{
    in &= RDRAM_MASK >> 1;
    if (rdram_valid_idx16(in)) {
...
static STRICTINLINE void
rdram_write_pair32(uint32_t in, uint32_t rval, uint8_t hval0, uint8_t hval1)
{
    in &= RDRAM_MASK >> 2;
    if (rdram_valid_idx32(in)) {
        rdram32[in] = rval;
...
static void
rdram_complete_delayed_hbwrites(int delayedhbwidx)
{
    if (rdram_valid_idx8((uint32_t)delayedhbwidx)) {
        int oldhbidx = delayedhbwidx >> 1;
